...
pub trait PredictNextActivityTestBalance {

    fn predict_next_activity_test_balance(
        &self,
...
impl PredictNextActivityTestBalance for EbiTraitStochasticSemantics {
    ...
    fn predict_next_activity_test_balance(
        ...
        match self {
            EbiTraitStochasticSemantics::Usize(sem) => sem.predict_next_activity_test_balance(event_log),
            EbiTraitStochasticSemantics::Marking(sem) => sem.predict_next_activity_test_balance(event_log),
            EbiTraitStochasticSemantics::NodeStates(sem) => sem.predict_next_activity_test_balance(event_log),
        }
    }

    // fn predict_next_activity_test_balance_with_fixed_prefix_length(
    //     &self,
    //     prefix_len: usize,
    //     event_log: &Box<dyn EbiTraitFiniteStochasticLanguage>,
    // ) -> Result<f64> {
    //     match self {
    //         EbiTraitStochasticSemantics::Usize(sem) => sem.predict_next_activity_test_balance_with_fixed_prefix_length(prefix_len,event_log),
    //         EbiTraitStochasticSemantics::Marking(sem) => sem.predict_next_activity_test_balance_with_fixed_prefix_length(prefix_len, event_log),
    //         EbiTraitStochasticSemantics::NodeStates(sem) => sem.predict_next_activity_test_balance_with_fixed_prefix_length(prefix_len, event_log),
    //     }
    // }

    // fn predict_next_activity_test_balance_with_large_prefix_length(
    //     &self,
    //     prefix_len: usize,
    //     event_log: Box<dyn EbiTraitFiniteStochasticLanguage>,
    // ) -> Result<f64> {
    //     match self {
    //         EbiTraitStochasticSemantics::Usize(sem) => sem.predict_next_activity_test_balance_with_large_prefix_length(prefix_len,event_log),
    //         EbiTraitStochasticSemantics::Marking(sem) => sem.predict_next_activity_test_balance_with_large_prefix_length(prefix_len, event_log),
    //         EbiTraitStochasticSemantics::NodeStates(sem) => sem.predict_next_activity_test_balance_with_large_prefix_length(prefix_len, event_log),
    //     }
    // }
}
...
impl<State: Displayable> dyn StochasticSemantics<StoSemState = State, SemState = State, AliState = State> {
    pub fn predict_next_activity_test_balance(
        &self,
        event_log: &Box<dyn EbiTraitFiniteStochasticLanguage>,
    ) -> Result<f64> {
        ...
        let mut balance2acc = HashMap::new();
        for i in 30..100 {
            let balance = i as f64 / 100.0; // balance between cost and probability
    ...
    // Both have activities - check if they match
        ...
        balance2acc.insert(i, accuracy);
        println!("accuracy for balance {}: {}", balance, accuracy);
    }

        for (balance_index, acc) in balance2acc.iter() {
            println!("Balance: {}, Average Similarity: {}", *balance_index as f64 / 100.0, acc);
        }
    //get the best acc regarding the balance from balance2acc
            for (balance_index, acc) in balance2acc.iter() {
            if *acc == balance2acc.values().cloned().fold(f64::MIN, f64::max) {
                println!("Best balance: {}, with the maximum Average Acc: {}", balance_index, acc);
            }
...
// impl<State: Displayable> dyn StochasticSemantics<StoSemState = State, SemState = State, AliState = State> {
//     pub fn predict_next_activity_test_balance_with_fixed_prefix_length(
//         &self,
//         prefix_len: usize,
//         event_log: &Box<dyn EbiTraitFiniteStochasticLanguage>,
//     ) -> Result<f64> {
...
// impl<State: Displayable> dyn StochasticSemantics<StoSemState = State, SemState = State, AliState = State> {
//     pub fn predict_next_activity_test_balance_with_large_prefix_length(
//         &self,
//         prefix_len: usize,
//         event_log: Box<dyn EbiTraitFiniteStochasticLanguage>,
//     ) -> Result<f64> {
