...
pub trait PredictSuffixTestBalance {
    fn predict_suffix_test_balance(
        &self,
        event_log: &Box<dyn EbiTraitFiniteStochasticLanguage>,
    ) -> Result<f64>;
}

impl PredictSuffixTestBalance for EbiTraitStochasticSemantics {
    fn predict_suffix_test_balance(
        &self,
        event_log: &Box<dyn EbiTraitFiniteStochasticLanguage>,
    ) -> Result<f64> {
        match self {
            EbiTraitStochasticSemantics::Usize(sem) => sem.predict_suffix_test_balance(event_log),
            EbiTraitStochasticSemantics::Marking(sem) => sem.predict_suffix_test_balance(event_log),
            EbiTraitStochasticSemantics::NodeStates(sem) => sem.predict_suffix_test_balance(event_log),
        }
    }
}   

impl<State: Displayable> dyn StochasticSemantics<StoSemState = State, SemState = State, AliState = State> {
    pub fn predict_suffix_test_balance(
        &self,
        event_log: &Box<dyn EbiTraitFiniteStochasticLanguage>,
    ) -> Result<f64> {
        ...
        let mut balance2dls = HashMap::new();
                    ...
                    if let Some(initial_state) = self.get_initial_state() {
                        ...
                        match astar_for_prediction::astar(&start, successors, heuristic, success) {
                            ...
                            //     let real_suffix_str = suffix.iter().map(|activity| activity.to_string()).collect::<Vec<_>>().join(", ");
                                ...
                                // println!("actual:{:?}, predicted:{:?}, maxlen: {}, current_dls:{}", suffix.clone(), predicted_suffix.clone(), max(suffix.len(), predicted_suffix.len()) as f64, current_dls);
        ...
        //     println!("({}, {})", key, *dls / prob as f64);
            balance2dls.insert(i, total_dls);
            println!("balance number: {}, Average dls: {}", i, total_dls);

        }
        // iterate balance2dls for each balance and its dls
        for (balance, dls) in &balance2dls {
            println!("Balance: {}, Average Similarity: {}", *balance as f64 / 100.0, dls);
        }

        // iterate balance2dls to find the balance with maximum dls
        for (balance_index, dls) in balance2dls.iter() {
            if *dls == balance2dls.values().cloned().fold(f64::MIN, f64::max) {
                println!("Best balance: {}, with the maximum Average similarity: {}", balance_index, dls);
            }
        }
